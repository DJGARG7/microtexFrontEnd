...
function App() {
  const [savedList, setSavedList] = useState(JSON.parse(localStorage.getItem("saved")));
  const [corpId,setCorpId] = useState();
  const [userId,setUserId] = useState();
  var data = [{ c_id: 'KSM0001', name: 'Dhananjay',u_id:'DJG0065' }, { c_id: 'SUF0001', name: 'Dhairya',u_id:'DIR0007' }]
  ...
  return (
    ...
    <div className="blackbox">
      <div className="leftPart">Saved Users
        <div>
          <table>
          {Object.keys(savedList).map((ele) => {
          return (
            <button onClick={()=>savedClickHandler(savedList[ele].c_id,savedList[ele].u_id)}>
              <tr>
      ...
      <div className="rightPart">Login
        <div>
        <form >
          <input type="text" name="cid" value={corpId}  placeholder="Corporate ID"/><br/>
          <input type="text" name="uid" value={userId}  placeholder="User ID"/><br/>
