  ...
  const loginHandler = (event) => {
    ...
    if(remember) {
      ...
      if(type === "proprietor" && !(userId in savedPro)){
        localStorage.setItem("savedPro", JSON.stringify({[userId]:{name:"fromPrompt",u_id:userId},...savedPro})); 
        setSavedPro({[userId]:{name:"fromPrompt",u_id:userId},...savedPro});
        console.log(savedPro)
      }
      else{
        localStorage.setItem("savedFirm", JSON.stringify({[userId]:{c_id:corpId,name:"fromPrompt",u_id:userId},...savedFirm})); 
        setSavedFirm({[userId]:{c_id:corpId,name:"fromPrompt",u_id:userId},...savedFirm});
          ...
          {Object.keys(savedFirm).map((ele) => {
            return (
              <button
                className="savedFirm"
                onClick={() =>
                  savedClickHandler(savedFirm[ele].c_id, savedFirm[ele].u_id)
                }
              >
                <table>
                  <tr>
                    <td rowSpan={2}>{savedFirm[ele].name}</td>
      ...
      <div className="rightPart">
        ...
        <div className="form">
          <form onSubmit={loginHandler}>
            ...
            />{" "}
            Remember me
            <button type="submit" className="btn btn-login" value="LOGIN">
              Login
            </button>
          </form>
        </div>
